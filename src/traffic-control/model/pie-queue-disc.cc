...
PieQueueDisc::InitializeParams (void)
{
  ...
  m_accuProb=0; // newley added variable
}
...
bool PieQueueDisc::DropEarly (Ptr<QueueDiscItem> item, uint32_t qSize)
{
  ...
  if (((m_qDelayOld.GetSeconds () < (0.5 * m_qDelayRef.GetSeconds ())) && (m_dropProb < 0.2))|| (QUEUE_DISC_MODE_BYTES && qSize <= 2 * m_meanPktSize))
    {
      return false;
    }
...
if (qDelay.GetSeconds () < (m_qDelayRef.GetSeconds ()*0.5) && m_qDelayOld.GetSeconds () < (m_qDelayRef.GetSeconds ()*0.5))
    {
      m_dropProb *= 0.98;
    }
